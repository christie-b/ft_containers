...
template <typename T>
void	printVecInfo(ft::vector<T> const &vct, bool print_content = 1)
{
...
int main()
{
	...
	}
	{
		...
		ft::vector<int> vec1(vector1.begin(), vector1.end());
	...
	std::cout << std::endl << BOLDCYAN << "Resize test" << RESET << std::endl;
	{
		...
		for (ft::vector<int>::size_type i = 1; i < 10; i++)
			vec.push_back(i);
	...
	{
		ft::vector<std::string> vec1(3, "abc");
		ft::vector<std::string> vec2(5, "xyz");
		std::cout << BLUE << "Vec1: \n" << RESET;
		printVecInfo(vec1);
		std::cout << BLUE << "\nVec2: \n" << RESET;
		printVecInfo(vec2);
	...
	}
	{
		...
		ft::map<int, char> map2(map1.begin(), map1.end());
	...
	std::cout << std::endl << BOLDCYAN << "Begin() / End() test through iteration" << RESET << std::endl;
	{
		...
		ft::map<int, char>::iterator ite = map1.end();
		while (it != ite)
		{
	...
	std::cout << std::endl << BOLDCYAN << "Rbegin() / Rend() test through iteration" << RESET << std::endl;
	{
		...
		map1[3] = 'd';
		map1[4] = 'e';
		ft::map<int, char>::reverse_iterator rit = map1.rbegin();
		ft::map<int, char>::reverse_iterator rite = map1.rend();
		while (rit != rite)
		{
	...
	std::cout << std::endl << BOLDCYAN << "Insert tests" << RESET << std::endl;
	{
		...
		map2.insert(ft::make_pair<int, char>(0, 'a'));
		map2.insert(ft::make_pair<int, char>(2, 'c'));
		map2.insert(ft::make_pair<int, char>(3, 'd'));
		...
		map2.insert(it, ft::make_pair<int, char>(1, 'b'));
	...
	std::cout << std::endl << BOLDCYAN << "value_comp test" << RESET << std::endl;
	{
		...
		ft::pair<char,int> highest = *map1.rbegin();          // last element
	...
	while (i < 1000000)
	{
		...
		mypair = ft::make_pair<int, int>(i, i);
		mymap.insert(mypair);
		i++;
	}
